...
class ApiService {
  ...
  static Future<ApiResult> requestEmailChangeOtp(String newEmail, String currentPassword) async {
    final token = await StorageService.getToken();
    final res = await http.post(
      Uri.parse(ApiConfig.changeEmailRequestOtp),
  ...
  /// Step 2 of email change: verifies the OTP and applies the new email.
  static Future<ApiResult> verifyEmailChangeOtp(String otp) async {
    final token = await StorageService.getToken();
    final res = await http.post(
      Uri.parse(ApiConfig.changeEmailVerifyOtp),
      headers: {'Content-Type': 'application/json', 'Authorization': 'Bearer $token'},
  ...
  /// Saves/updates the user's Sri Lankan mobile number.
  static Future<ApiResult> updateMobile(String mobile) async {
    final token = await StorageService.getToken();
    final res = await http.post(
      Uri.parse(ApiConfig.updateMobile),
      headers: {'Content-Type': 'application/json', 'Authorization': 'Bearer $token'},
